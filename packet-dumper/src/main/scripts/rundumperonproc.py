...
def main(argv):
    ...
    for opt, arg in opts:
        if opt == '-h':
            print 'rundumperonproc.py --kafka-bootstrap-servers host:port[,host:port,...]'
            sys.exit()
        elif opt in "--kafka-bootstrap-servers":
            kafka_bootstrap_servers = arg
    ...
    for pid in unique_pids_to_process:
        with open("##log_path##/packet-dumper_" + str(pid) + ".log","wb") as out:
            ...
            subprocess.Popen(["nsenter", "-t", str(pid), "-n", "java", "-jar", "/opt/cisco/##software_name##/packet-dumper-##software_version##.jar", portFilter, "--kafka.bootstrap-servers=" + kafka_bootstrap_servers, "--spring.profiles.active=kafka,vm,vmCPU", "--kafkaConnector.logMode=false", "--pcap.devicePrefix=eth", "--server.port=0", "--LISTEN_PID={0}".format(pid)], stdin=out, stdout=out, stderr=out)
